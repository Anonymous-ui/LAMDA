...
def calculate_weights(lines):
    weights = [0] * len(lines)  # Initialize all weights to 0

    input_ids = {}  # Maps input_id to list of line indices
    output_ids = {}  # Maps line index to output_id
    ...
    # Update weights based on future dependencies
...
def dpprocess_conversation(item, item_index, total_items, max_token_limit=1000):
    ...
    print("Final weights for each line:")
    for idx, weight in enumerate(weights):
        print(f"Line {idx + 1}: {weight}")
    ...
    while i >= 0:
        ...
        if start != -1:
            ...
            print(f"Backtracking: Current cut from line {start + 1} to line {i + 1}")
        i = start - 1
    ...
    print(f"Processing item {item_index + 1}/{total_items} completed. Split into {len(segments)} parts.")
    return segments
    ...
    try:
        ...
        for item_index, item in enumerate(data):
            ...
            for part_number, segment in enumerate(segments, start=1):
                ...
                new_id += 1
            print(f"Processed {item_index + 1}/{total_items} items.")
