...
class Trainer(object):
    ...
    def init_model(self):
        ...
        if self.args.pretrain:
            ...
            ckpt = torch.load(self.args.pretrain, map_location='cuda:0')
    ...
    def test(self):
        ...
        with torch.no_grad():
            for iteration, batch in enumerate(self.test_loader):
                ...
                #print('batch',batch)
                data1, data2, n_edges, y = self.unpack_batch(batch)
                #print("data1", data1, 'data2', data2)  # 通过print可以判断出是GNN的编码格
                # NNLQP: data1=data, data2=static feature
                # NasBench101/201: data1=netcode, data2=adjacency matrix
                time_i_1 = time.time()
                pred_cost = self.model(data1, data2, n_edges)
                content = f"test pred_cost {pred_cost} label: {y}"
                with open("D:\\NAR\\NAR-Former-V2-main\\NAR-Former-V2-main\\dataset\\unseen_structure\\result\\lama.txt",
                          "a") as file:
                    file.write(content + "\n")
