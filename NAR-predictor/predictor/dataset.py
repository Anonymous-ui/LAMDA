...
def get_torch_data(onnx_file, batch_size, cost_time, embed_type):#用于从给定的 onnx 文件中提取图特征onnx_file: 输入的 onnx 文件路径，batch_size: 批次大小，cost_time: 消耗时间，embed_type: 嵌入类型。
...
class AccuracyDataset(TorchDataset):
    def __init__(self, dataset, part, data_path, percent=0, use_aug=False, dx=32, dp=32, embed_type=None,
                 op_code_type='pe'):
        self.dx = dx
        self.dp = dp
        ...
        for key in keys:
            ...
            if self.use_aug and self.part == 'train':
                ...
                if len(auged_adjs) > 0:
                    ...
                    auged_code, auged_adj = padding_for_batch(auged_code,
                                                              auged_adjs[0]) if self.dataset == 'nasbench101' \
                    ...
                    # !!!!
                    adj_tensor += torch.t(adj_tensor.clone())
    ...
    def preprocess(self, data):
        ...
        # !!!!!
        adj += torch.t(adj.clone())
        with open('C:\\Users\\UserName\\Desktop\\ut.txt', 'w', encoding='utf-8') as file:
            file.write("nas201 code: {}\n".format(code))
            file.write("nas201 adj: {}\n".format(adj))
            file.write("nas201 N: {}\n".format(N))
            file.write("nas201 V_label: {}\n".format(V_label))
            file.write("nas201 T_label: {}\n".format(T_label))

        print("内容已成功保存到output.txt文件中。")
        return code, adj, N, V_label, T_label


class GraphLatencyDataset(Dataset):
    # specific a platform
    def __init__(self, root, onnx_dir, latency_file, embed_type, selfpos, override_data=False, transform=None,
                 pre_transform=None,
    ...
    def get(self, idx):
        ...
        #print('data',data, 'sf',sf, 'n_edges',n_edges, 'graph_name',graph_name, 'plt_id',plt_id)
